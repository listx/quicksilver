...
_RTW_EDB_FUNCS = addTrueTest [r1, r2, r2']
	...
	mausoleumEffects = concatMap (\s -> (replicate 12 ' ') ++ s ++ "\r\n")
		["faction_capability"
		, "{"
		, "    construction_time_bonus_military bonus -20" ++ reqm
		, "    construction_time_bonus_defensive bonus -20" ++ reqm
		, "    construction_time_bonus_religious bonus -20" ++ reqm
		, "    construction_time_bonus_other bonus -20" ++ reqm
		, "    population_health_bonus bonus 4" ++ reqm
		, "}"
		]
		where
		reqm = " requires hidden_resource mausoleum"
...
_RTW_EDU_FUNCS = addTrueTest unitTurns
	...
	r1 =
		[ ("^soldier.+?", id) -- we can lead with this generic regex b/c the text is broken up into 1-unit blocks (no danger of matching over into the next unit(s))
