...
_M2TW_DC_FUNCS = addTrueTest [r1, r2, r3] ++ addTrueTest assassin
	...
	r1 =
		[ ("^type\\s+admiral.+?starting_action_points\\s+", id)
		, (multRoundInt 3)
		]
	-- Diplomat movement speed 2x
	r2 =
		[ ("^type\\s+diplomat.+?starting_action_points\\s+", id)
		, (multRoundInt 2)
		]
	-- Campaign movement speed 1.75x
	r3 =
		[ ("^starting_action_points\\s+", id)
		, (multRoundInt 1.75)
		]
	assassin =
		[
...
_M2TW_DCD_FUNCS = addTrueTest [agent, rebels]
	++ addTrueTest (witchesHereticsInquisitors ++ assassin)
	where
...
_M2TW_DCAD_FUNCS = addTrueTest [r1]
	...
	-- easily)
	r1 =
		[ ("papal_faction.+?invasion_decisions.+?", id)
		, ("<decision_entry>.+?", id)
...
_M2TW_DFS_FUNCS = addTrueTest [rN]
	...
	rN =
		[ ("^;Trigger 0102_city_razed.+?;-+", nil)
		]

_M2TW_DM_FUNCS :: RegexSets
_M2TW_DM_FUNCS = addTrueTest [r1, r2, r3, r4a, r4b, r5, r6a, r6b, r4']
	++ addTrueTest (merchantsGuild ++ thievesGuild)
	where
...
_M2TW_DS_FUNCS = addTrueTest [r1, r2, r3]
	...
	r1 =
		[ ("^brigand_spawn_value\\s+", id)
		, (multRoundInt 20)
		]
	-- Pirate spawn rate 20x lower
	r2 =
		[ ("^pirate_spawn_value\\s+", id)
		, (multRoundInt 20)
		]
	-- King's purse 2x
	r3 =
		[ ("^denari_kings_purse\\s+", id)
		, (multRoundInt 2)
		]
	-- Replace all gold resources with chocolate in the map, except for
	-- Aztecs
	r4 =
		[ ("^resource\\s+", id, alwaysTrue)
	...
	giveRoads =
		...
		,
			[ ("^\\s+building\\r\\n\\s+\\{\\r\\n\\s+type\\s+"
			++ "hinterland_castle_roads\\s+c_roads\\r\\n\\s+\\}\\r\\n", nil)
			]
		,
...
_M2TW_DSK_FUNCS = addTrueTest [r1, r2, r3]
	...
	r1 =
		[ ("^anim\\s+conduct_diplomacy.+?\\r\\n", nil)
		]
	-- Diplomats: remove needless bowing animation (same as deselect animation)
	r2 =
		[ ("^anim\\s+selected_to_stand[^\\r]+?Stratmap_Diplomat.+?\\r\\n", nil)
		]
	-- Princesses: remove needless bowing animation
	r3 =
		[ ("^anim\\s+selected_to_stand[^\\r]+?Stratmap_Princess.+?\\r\\n", nil)
		]
...
_M2TW_DW_FUNCS = addTrueTest [r1, r2, r3, r4, r5, r6, r7, r8, rN]
	...
	r1 =
		[ ("^\\s+gate\\s.+?full_health\\s", id)
		, (multRoundInt 5)
		]
	r2 =
		[ ("^\\s+wall.+?full_health\\s", id)
		, (multRoundInt 5)
		]
	r3 =
		[ ("^\\s+gateway.+?full_health\\s", id)
		, (multRoundInt 5)
		]
	-- Tower HP 2x
	r4 =
		[ ("^\\s+tower.+?full_health\\s", id)
		, (multRoundInt 2)
		]
	-- Tower non-flaming firing rate 2x (i.e., reduce firing delay by 0.5x)
	r5 =
		[ ("^\\s+fire_rate\\s+small\\s+", id)
		, (multRoundInt 0.5)
		]
	r6 =
		[ ("^\\s+fire_rate\\s+normal\\s+", id)
		, (multRoundInt 0.5)
		]
	r7 =
		[ ("^\\s+fire_rate\\s+large\\s+", id)
		, (multRoundInt 0.5)
		]
	r8 =
		[ ("^\\s+fire_rate\\s+huge\\s+", id)
		, (multRoundInt 0.5)
		]
	-- City/Castle defense tower activation range 8x
	rN =
		[ ("^\\s+control_area_radius\\s+", id)
		, (multRoundInt 8)
		]
...
_M2TW_EDB_FUNCS = addTrueTest
	...
	r4 =
		[ ("^\\s{8}motte_and_bailey.+?wall_level.+?", id)
		, (_REGEX_INT, upkeep "1")
		]
	r5 =
		[ ("^\\s{8}wooden_castle.+?wall_level.+?", id)
		, (_REGEX_INT, upkeep "2")
		]
	r6 =
		[ ("^\\s{8}castle.+?wall_level.+?", id)
		, (_REGEX_INT, upkeep "3")
		]
	r7 =
		[ ("^\\s{8}fortress.+?wall_level.+?", id)
		, (_REGEX_INT, upkeep "4")
		]
	r8 =
		[ ("^\\s{8}citadel.+?wall_level.+?", id)
		, (_REGEX_INT, upkeep "5")
		]
	-- All free upkeep slots 2x
	r9 =
		[ ("^\\s+free_upkeep\\s+bonus\\s+", id)
		, (multRoundInt 2)
		]
	upkeep :: String -> (BC.ByteString -> BC.ByteString)
	upkeep amt = (\d -> BC.append d (BC.pack $ "\r\n" ++ replicate 16 ' '
		++ "free_upkeep bonus " ++ amt))
	-- Disable the "Mining network" building.
	r10 =
		[ ("^building\\s+hinterland_mines.+?levels\\s+mines", id)
		, ("\\s+mines\\+1[^\\r]+", nil)
		, (".+?\\r\\n        {\\r\\n", id)
		, ("\\s+convert_to.+?\\r\\n", nil)
		, (".+?upgrades.+?\\{\\r\\n", id)
	...
	r11 =
		[ ("^building\\s+hinterland_castle_mines.+?levels\\s+c_mines", id)
		, ("\\s+c_mines\\+1[^\\r]+", nil)
		, (".+?\\r\\n        {\\r\\n", id)
		, ("\\s+convert_to.+?\\r\\n", nil)
		, (".+?upgrades.+?\\{\\r\\n", id)
	...
	r12 =
		[ ("^\\s+mines\\s+city\\s+requires\\s+factions.+?resource\\s+gold", id)
		, ("[^\\r]+", nil)
		]
	r13 =
		[ ("^\\s+c_mines\\s+castle\\s+requires\\s+factions.+?resource\\s+gold"
			, id)
		, ("[^\\r]+", nil)
		]
	...
	r16 =
		[ ("^building\\s+guild_swordsmiths_guild.+?heavy_cavalry_bonus.+?"
		++ "heavy_cavalry_bonus\\s+bonus\\s+", id)
		, (_REGEX_INT, only "2")
		]
	...
	assassin = replicate 16 ' ' ++ (concat
		[ "agent assassin  0  requires factions"
		, " { northern_european, middle_eastern, eastern_european, greek,"
		, " southern_european, } \r\n"
		]
		)
	tradeFleet n = replicate 16 ' ' ++ "trade_fleet " ++ n ++ "\r\n"
	tradeBaseIncomeBonus n = replicate 16 ' '
	...
	recruitmentsBasic = map makeRegexSnippets
		. zip (repeat "\\r\\n.+?            \\{\\r\\n") $ concat
		[ zip3 (repeat Core) [[village, town, largeTown, city, largeCity]]
			(repeat "^\\s+wall_level")
		, zip3 (repeat Castle) [[village, town, largeTown, city, largeCity]]
			(repeat "^\\s+wall_level")
		, zip3 (repeat Equestrian) [[village, town, largeTown, city, largeCity]]
			(repeat "^\\s+}")
		, zip3 (repeat Barracks)
			[[village, town, largeTown, city, largeCity, hugeCity1, hugeCity2]]
			(repeat "^\\s+}")
		, zip3 (repeat CastleBarracks)
			[[village, town, largeTown, city, largeCity]] (repeat "^\\s+}")
		, zip3 (repeat ProfessionalMilitary) [[hugeCity1, hugeCity2]]
			(repeat "^\\s+}")
		, zip3 (repeat Missiles) [[town, largeTown, city, largeCity]]
			(repeat "^\\s+}")
		, zip3 (repeat Siege) [[largeTown, city, largeCity]] (repeat "^\\s+}")
		, zip3 (repeat CastleSiege) [[largeTown, city, largeCity]]
			(repeat "^\\s+}")
		, zip3 (repeat Cannon) [[city, largeCity, hugeCity1, hugeCity2]]
			(repeat "^\\s+}")
		, zip3 (repeat CastleCannon) [[city, largeCity, hugeCity1, hugeCity2]]
			(repeat "^\\s+}")
		, zip3 (repeat UrbanEquestrian) [[city, largeCity]] (repeat "^\\s+}")
		, zip3 (repeat Port) [[largeTown, city, largeCity, hugeCity1]]
			(repeat "^\\s+}")
		, zip3 (repeat Caravan) [[largeTown, largeCity]]
			(repeat "^\\s+trade_level")
		]
	recruitmentsBasicGuilds = map makeRegexSnippets
		. zip (repeat ".+?capability.+?\\{\\r\\n") $ concat
		[ zip3 (repeat AssassinsMuslimGuild) [[city, largeCity, hugeCity1]]
			(repeat "^\\s+}")
		, zip3 (repeat MasonsGuild) [[city, largeCity, hugeCity1]]
			(repeat "^\\s+construction")
		, zip3 (repeat MerchantsGuild) [[city, largeCity, hugeCity1]]
			(repeat "^\\s+trade_base")
		, zip3 (repeat TemplarsChapterHouse) [[city, largeCity, hugeCity1]]
			(repeat "^\\s+}")
		, zip3 (repeat StJohnsChapterHouse) [[city, largeCity, hugeCity1]]
			(repeat "^\\s+}")
		, zip3 (repeat TeutonicKnightsChapterHouse)
			[[city, largeCity, hugeCity1]]
			(repeat "^\\s+}")
		, zip3 (repeat KnightsOfSantiagoChapterHouse)
			[[city, largeCity, hugeCity1]]
			(repeat "^\\s+}")
		]
	makeRegexSnippets (startpoint, (building, levels, endpoint)) =
		[buildingRegex]
	...
	recruitment =
		...
		,
			...
			[ ("^building\\s+market.+?capability.+?capability.+?capability.+?"
				, id)
		...
		,
			...
			[ ("^building\\s+guild_assassins_guild.+?capability.+?"
				++ "capability.+?\\{\\r\\n", id)
			, ("\\s+recruit_pool.+?\\r\\n"
		...
		,
			...
			[ ("^building\\s+guild_woodsmens_guild.+?capability.+?"
				++ "capability.+?\\{\\r\\n", id)
			, ("\\s+recruit_pool.+?\\r\\n"
...
_M2TW_EDU_FUNCS = addTrueTest [r1, r2, r3] ++ [rN]
	...
	r1 =
		[ ("^dictionary\\s+\\w+?_Bodyguard.+?"
			++ "soldier\\s+\\w+?_Bodyguard,\\s+", id)
	...
	r2 =
		[ ("^category\\s+infantry\\s+class\\s+missile.+?stat_pri"
			++ "\\s+.+?,.+?,.+?,.+?,\\s+", id)
		, (multRoundInt 2)
		]
	-- Pikemen units: fix rubber swords bug
	r3 =
		[ ("^stat_pri_attr\\s+.+?,\\s+long_pike.+?", id)
		, ("^stat_sec.+?\\r\\n", only $ "stat_sec         0, 0, no, 0 0, no, "
			++ "melee_simple, blunt, none, 25, 1\r\n")
		]
	-- Basic infantry have free upkeep (mental up to 5 AND cost up to 650)
	rN =
		[ ("^category\\s+infantry.+?", id, alwaysTrue)
		, ("^attributes[^\\r]+", add ", free_upkeep_unit"
			, \s -> not $ BC.isInfixOf (BC.pack "free_upkeep_unit") s)
